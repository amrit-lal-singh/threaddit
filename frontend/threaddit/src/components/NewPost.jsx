...
export default function NewPost({
  ...
  const [thread, setThread] = useState(
    isEdit ? { id: threadInfo.thread_id, name: threadInfo.thread_name } : false
  );
    ...
    mutationFn: async (e) => {
      ...
      if (!isEdit) {
        await axios
          ...
          .catch((err) =>
            alert(`${err.message} check your fields, Title is mandatory`)
          );
      } else {
        await axios
          .patch(`/api/post/${postInfo.id}`, formData, {
            headers: { 'Content-Type': 'multipart/form-data' },
          })
          .then((res) => {
            queryClient.setQueryData(
              { queryKey: ['post/comment', `${postInfo.id}`] },
              (oldData) => {
          ...
          .catch((err) =>
            alert(
              `${err.message} check your fields, Title and thread is mandatory`
            )
          );
      }
    },
