    ...
    mutationFn: async (has_subscribed) => {
      if (has_subscribed) {
        axios.delete(`/api/threads/subscription/${threadData.id}`).then(() => {
            ...
            mixpanel.track('subscribed', { total_subscriber: threadData.subscriberCount });
        });
      } else {
          ...
          >
            ...
            <div className="flex flex-col flex-1 justify-around items-center p-2 space-y-1">
              ...
              {threadData?.description && (
                <p
                  className={`text-center py-4 md:py-2 text-sm ${threadData?.description.length > 90 && 'text-xs'}`}
                >
                  {threadData?.description}
